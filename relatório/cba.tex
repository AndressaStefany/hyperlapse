...
onde os $\lambda$ são os autovalores da matriz M. Esse valor precisa ser maior que um limiar, um $\lambda_min$. Observe a figura abaixo: %(harris_detector)

\begin{figure}[h]
	\centering
		\graphicspath{ {shitomasi} }		
		\includegraphics[width=7cm]{shitomasi_space.png}
		\caption{Método Shi-Tomasi}
		\label{fig:shitomasi}
		\cite{harvard} %py_shi_tomasi
\end{figure}

A janela só será considerada um canto quando $\lambda_1$ e $\lambda_2 $ estiverem acima do $\lambda_min$, ou seja, quando estiver na região verde mostrada na figura \ref{fig:shitomasi}.

\begin{itemize}
\item calcOpticalFlowPyrLK()
\end{itemize}

Calcula o fluxo óptico para um conjunto de características esparsas usando o método não-iterativo Lucas Kanade, o qual assume que o fluxo é constante em pequenas janeças de tamanho mxm e com m>1. Numerando os pixels de 1 a n chega-se no seguinte conjunto de equações
\begin{center}
...
Um exemplo visual é a figura \ref{fig:image}, é passado como parâmetro a imagem 1, imagem 2 e as bordas da imagem 2, o retorno será as bordas da imagem 2.

\begin{figure}[h]
	\centering
		\graphicspath{ {image} }		
		\includegraphics[width=7cm]{Warp_Affine_Tutorial_Theory_0.jpg}
...
Sendo pré-estabelecido dois vetores de coordenadas de pontos ótimos de borda, essa função retornará um transformação afim, isto é, retornará o deslocamento em $x$, $y$ e $\theta$, como mostrado nas matrizes abaixo:
...
Basicamente, essa função irá aplicar uma matriz de transformação afim a uma imagem, onde essa essa transformação afim possuirá deslocamento em \textit{x}, em \textit{y}, o escalonamento e a rotação, como mencionado anteriormente. Essa moficação é realizada na imagem através da seguinte equação: 
